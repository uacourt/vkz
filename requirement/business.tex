...
\subsubsection{Важкий клієнт фіксації засідань (Flutter Offline Client) [Резервний компонент]}
Автономний десктопний/мобільний додаток на базі фреймворку Flutter (запланований у наступних фазах розширення системи) для локальної фіксації (запису та протоколювання) судових засідань в умовах відсутності зв’язку з мережею Інтернет. Додаток інтегрується з платами відеозахвату (BlackMagic) та OBS Studio і використовує локальне реляційне сховище (SQLite/LSM).

\subsubsection{Бекенд лінія масштабування (Backend VM Nodes)}
Сукупність масштабованих вузлів віртуальних машин (PHP 8.1 / Apache), які обробляють запити до бізнес-логіки системи та підключаються до реляційної бази даних PostgreSQL.

\subsubsection{Фронтенд лінія масштабування (Frontend VM Nodes)}
Масштабовані веб-сервери (Nginx / Apache), які забезпечують доставку статичних файлів інтерфейсу, кешування та стрімінг файлів відеофонограм зі сховища Scality.
...
\subsubsection{TURN-сервери (TURN Server Pool)}
Пул з TURN-серверів для проходження трафіку WebRTC через NAT та брандмауери в закритих судових мережах, що конфігурується статично на рівні сигналізації та медіа-сервера.
...
\subsubsection{Реляційне сховище протоколів та чатів (PostgreSQL Database)}
База даних PostgreSQL (Master з гарячою Standby реплікою на РЦОД), що містить логі чатів конференцій та таблиці дій протоколу.

\subsubsection{Система вбудованого моніторингу (Monitoring API)}
Модуль, вбудований у сервер застосунків OpenVidu, що збирає статистику активних медіа-кімнат та поточного навантаження.

\subsubsection{Сервер збору метрик (Monitoring Collector Node)}
Виділений модуль, який збирає агреговані метрики навантаження і передає їх до системи моніторингу (Zabbix) за протоколом HTTP Basic Auth.
...
Щомісячний запуск завдання з видалення застарілих логів дій адміністратора, від дати створення яких пройшло більше 12 місяців (365 днів).
